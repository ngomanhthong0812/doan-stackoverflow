...
exports.createComment = async (req, res, next) => {
  try {
    ...
    if (parentType === "Answer" && !parentComment) {
      await Answer.findByIdAndUpdate(parentId, { $inc: { commentsCount: 1 } });
    }
...
exports.getCommentsByAnswer = async (req, res, next) => {
  try {
    const comments = await commentService.getCommentsByAnswer(
      req.params.answerId
    );
    res.json(comments);
  } catch (err) {
...
exports.getComments = async (req, res, next) => {
  try {
    const { parentType, parentId, page, perPage } = req.query;
