...
function RouteComponent() {
  ...
  const badges = {
    gold: user?.badges?.filter((b) => b.type === "gold") || [],
    silver: user?.badges?.filter((b) => b.type === "silver") || [],
    bronze: user?.badges?.filter((b) => b.type === "bronze") || [],
  };
  ...
  return (
    <div className="max-w-5xl mx-auto mt-10 space-y-8">
      ...
      <div className="flex items-center gap-6">
        <img
          src={
            user?.avatar || "https://avatars.githubusercontent.com/u/000000?v=4"
          }
          alt="avatar"
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div className="p-2 bg-white shadow rounded border text-center">
          <p className="text-md font-bold">{user?.reputation}</p>
          <p className="text-xs text-gray-500">Reputation</p>
        </div>
        <div className="p-2 bg-white shadow rounded border text-center">
          <p className="text-md font-bold">{user?.answersCount || 0}</p>
          <p className="text-xs text-gray-500">Answers</p>
        </div>
        <div className="p-2 bg-white shadow rounded border text-center">
          <p className="text-md font-bold">{user?.questionsCount || 0}</p>
          <p className="text-xs text-gray-500">Questions</p>
        </div>
        <div className="p-2 bg-white shadow rounded border text-center">
          <p className="text-md font-bold">{user?.followers?.length || 0}</p>
      ...
      <div>
        ...
        <div className="grid grid-cols-4 gap-6">
          ...
          <div className="flex items-center gap-2 border p-2 rounded-sm">
            <BadgeIcon type="gold" />
            <div className="text-sm flex flex-col leading-2">
              <span className="font-semibold text-lg">
                {badges.gold.length}
              </span>
              <span>
                gold badge
                {badges.gold.length > 1 ? "s" : ""}
              </span>
            </div>
          </div>
